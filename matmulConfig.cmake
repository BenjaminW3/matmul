...
SET(MATMUL_INDEX_TYPE "int" CACHE STRING "The index type.")
LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_INDEX_TYPE=${MATMUL_INDEX_TYPE}")
...
ENDIF()
OPTION(MATMUL_BUILD_PAR_CUDA_MEMCPY_FIXED_BLOCK_SIZE "Enable the GEMM algorithm from the CUDA developers guide with fixed block size" OFF)
IF(MATMUL_BUILD_PAR_CUDA_MEMCPY_FIXED_BLOCK_SIZE)
    SET(_MATMUL_BUILD_CUDA ON)
    SET(MATMUL_BUILD_PAR_CUDA_FIXED_BLOCK_SIZE ON CACHE BOOL "" FORCE)
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_PAR_CUDA_MEMCPY_FIXED_BLOCK_SIZE")
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_CUDA_MEMCPY")
ENDIF()
OPTION(MATMUL_BUILD_PAR_CUDA_FIXED_BLOCK_SIZE "Enable the GEMM algorithm from the CUDA developers guide with fixed block size" OFF)
IF(MATMUL_BUILD_PAR_CUDA_FIXED_BLOCK_SIZE)
    SET(_MATMUL_BUILD_CUDA ON)
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_PAR_CUDA_FIXED_BLOCK_SIZE")
ENDIF()
OPTION(MATMUL_BUILD_PAR_CUDA_MEMCPY_DYN_BLOCK_SIZE "Enable the GEMM algorithm from the CUDA developers guide with dynamic block size" OFF)
IF(MATMUL_BUILD_PAR_CUDA_MEMCPY_DYN_BLOCK_SIZE)
    SET(_MATMUL_BUILD_CUDA ON)
    SET(MATMUL_BUILD_PAR_CUDA_DYN_BLOCK_SIZE ON CACHE BOOL "" FORCE)
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_PAR_CUDA_MEMCPY_DYN_BLOCK_SIZE")
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_CUDA_MEMCPY")
ENDIF()
OPTION(MATMUL_BUILD_PAR_CUDA_DYN_BLOCK_SIZE "Enable the GEMM algorithm from the CUDA developers guide with dynamic block size" OFF)
IF(MATMUL_BUILD_PAR_CUDA_DYN_BLOCK_SIZE)
    SET(_MATMUL_BUILD_CUDA ON)
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_PAR_CUDA_DYN_BLOCK_SIZE")
ENDIF()
OPTION(MATMUL_BUILD_PAR_BLAS_CUBLAS_MEMCPY "Enable the GEMM using the NVIDIA cublas2 implementation." OFF)
IF(MATMUL_BUILD_PAR_BLAS_CUBLAS_MEMCPY)
    SET(_MATMUL_BUILD_CUBLAS ON)
    SET(MATMUL_BUILD_PAR_BLAS_CUBLAS ON CACHE BOOL "" FORCE)
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_PAR_BLAS_CUBLAS_MEMCPY")
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_CUDA_MEMCPY")
ENDIF()
...
ENDIF()
OPTION(MATMUL_BUILD_PAR_ALPAKA_ACC_GPU_CUDA_MEMCPY "Enable the GEMM using the alpaka CUDA accelerator" OFF)
IF(MATMUL_BUILD_PAR_ALPAKA_ACC_GPU_CUDA_MEMCPY)
    SET(_MATMUL_BUILD_ALPAKA ON)
    SET(ALPAKA_ACC_GPU_CUDA_ENABLE ON CACHE BOOL "" FORCE)
    SET(_MATMUL_BUILD_CUDA ON)
    SET(MATMUL_BUILD_PAR_ALPAKA_ACC_GPU_CUDA ON CACHE BOOL "" FORCE)
    LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_BUILD_PAR_ALPAKA_ACC_GPU_CUDA_MEMCPY")
ELSE()
...
#-------------------------------------------------------------------------------
IF(MATMUL_BUILD_PAR_CUDA_FIXED_BLOCK_SIZE)
    SET(MATMUL_CUDA_FIXED_BLOCK_SIZE 32 CACHE INTEGER "The block size used on the gpu.")
    IF(MATMUL_CUDA_FIXED_BLOCK_SIZE)
        LIST(APPEND _MATMUL_COMPILE_DEFINITIONS_PUBLIC "MATMUL_CUDA_FIXED_BLOCK_SIZE=${MATMUL_CUDA_FIXED_BLOCK_SIZE}")
    ENDIF()
ENDIF()
